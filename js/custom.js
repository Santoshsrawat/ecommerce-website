...
function manage_cart(pid, type, is_checkout) {
    var is_error = '';
    var qty = (type === 'update') ? jQuery("#" + pid + "qty").val() : jQuery("#qty").val();
    ...
    if (is_error === '') {
        // Assuming you have an AJAX endpoint like 'manage_cart.php'
        jQuery.ajax({
            url: 'manage_cart.php',  // Replace with your actual endpoint
            type: 'post',
            data: {pid: pid, type: type, qty: qty, cid: cid, sid: sid},
            success: function(result) {
                result = result.trim();
...
// Example AJAX request for updating cart quantity
