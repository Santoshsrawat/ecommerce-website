...
function cart_add($con, $uid, $pid, $qty, $attr_id = null) {
    $added_on = date('Y-m-d H:i:s');

    $stmt = mysqli_prepare($con, "INSERT INTO cart (user_id, product_id, quantity, attr_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "iiisss", $uid, $pid, $qty, $attr_id, $added_on, $added_on);
